...
;(function ($, window, document, undefined) {
	...
	*/
    ...
    // Send back the plugin instance
    options.onInit(this);

		// Resize event listener initial state 
		this.resizeInitialised = false,

		// Call the init method to set up the instance
		this.init();
    ...
    */
    registerEventListener: function(options) {

      var _this = this;

      // Listen for any clicks requesting panel open or close
      options.view.find('.panel-open, .panel-close').on('click', function(e, ref) {
    ...
    */
      ...
      */
      ...
      // Vertical panels
        ...
        // TODO: we need to make sure content is loaded into panel before we do this
        setTimeout(function() {
          ...
          heightBreakpoint = Math.floor(windowHeight / 100 * _this.settings.verticalPanelMaxHeight);
      ...
      if (options) {
        ...
        if (options.requestedPanel) {
          ...
          if (options.panelReference) {
            ...
            var isOpen = options.requestedPanel.attr('data-open') === 'true';

            // Only close the panel if it is open
            if (isOpen) {
